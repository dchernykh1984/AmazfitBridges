...
//
// `--no-verify` skips every pre-commit hook - Prettier, ESLint, the ASCII guard
// and the Conventional Commits check - so a commit made with it fails in CI
// instead, one push later. If a single hook cannot run on this machine, skip that
// one by name: `SKIP=<hook-id> git commit -m '...'`.
//
// Force-pushing rewrites published history. `main` is a linear-history branch and
// nothing here has ever needed it.
//
// Wired as a PreToolUse hook on Bash: exit 2 blocks the call and shows stderr to
// the agent, which can then run the corrected command.
let input = "";
...
process.stdin.on("end", () => {
  ...
  if (/\bgit\s+(?:-\S+\s+)*commit\b[^\n;|&]*(?:--no-verify|\s-\w*n\w*\b)/.test(code)) {
    refuse(
      "--no-verify disables every pre-commit hook, not just the one that is in " +
        "the way.\nSkip that one by name instead:\n" +
...
// pull request body - carries them as ordinary text inside `<<'EOF' ... EOF`, and
// matching there would block writing this very file. Only what the shell would
// execute is worth inspecting.
