...
import { isForcedSolvable, solvePuzzle } from "./solver.js";
...
export function generatePuzzle(config, seed) {
  ...
  for (let attempt = 0; attempt < config.attempts; attempt++) {
    ...
    const result = solvePuzzle(candidate.puzzle, { limit: 2, maxNodes: config.maxNodes });
    const unique = !result.exhausted && result.count === 1;
    if (unique && isForcedSolvable(candidate.puzzle)) {
