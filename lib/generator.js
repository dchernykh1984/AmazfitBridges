...
import { buildPuzzle, edgeBetween, isValidSolution, MAX_REQUIRED } from "./puzzle.js";
import { hasUniqueSolution, isForcedSolvable } from "./solver.js";
import { isPlayable, playableCells } from "./playfield.js";
...
// already spanned by a bridge, and not touching another island - see MIN_SPAN.
// The roundness is what keeps a puzzle off the corners of its own grid, where a
// round watch has no glass to draw on.
function canHostIsland(grid, col, row) {
  if (!isPlayable(grid.cols, grid.rows, col, row)) {
    return false;
  }
...
// free, so one of the game's three rules never does any work at all. Closing a
// loop is what forces a player to think about connectivity.
//
// New pairs come first and doubling second, deliberately. Doubling an existing
// pair is far easier to find - the lane is already clear, because it is the
// pair's own lane - and a generator that takes the easy option ends up with
// every bridge doubled and not one loop on the board. That was measured: 88 per
...
// Join two islands that are already placed but not yet linked. Each one of these
// closes a loop, because the layout was grown as a tree.
function closeLoops(grid, config, random) {
  const attempts = config.extraBridges * 4;
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    for (let d = 0; d < directions.length; d++) {
      ...
      if (grid.links.has(key) || target.blockedBy.size > 0) {
        continue;
      }
...
// one-or-two vocabulary rather than settling on one of them. Kept to a share of
// the links rather than a free-for-all: a board of all doubles reads as bundles
// of tram lines and solves itself.
function doubleSomeBridges(grid, config, random) {
  const keys = shuffle(random, Array.from(grid.links.keys()));
  const wanted = Math.round(keys.length * config.doubleShare);
  let doubled = 0;

  for (let i = 0; i < keys.length && doubled < wanted; i++) {
    if (grid.links.get(keys[i]) !== 1) {
      continue;
    }
    const parts = keys[i].split(":");
    const a = Number(parts[0]);
    const b = Number(parts[1]);
    if (grid.degrees[a] >= MAX_REQUIRED || grid.degrees[b] >= MAX_REQUIRED) {
      continue;
    }
    addBridges(grid, a, b, 1);
    doubled += 1;
  }
}
...
export function generateCandidate(config, seed) {
  ...
  const cells = playableCells(config.cols, config.rows);
  if (cells.length === 0) {
    return null;
  }
  const start = cells[randomInt(random, cells.length)];
  addIsland(grid, start.col, start.row);
...
// A playable board for the given size. Candidates are generated from
