...
// Whether any part of a box is on screen. The page culls board widgets with it
// while the map is dragged: a bridge that has been panned off the edge does not
// need its position updated, and on a board twice the width of the watch that is
// more than half the work saved on every frame.
export function boxIntersectsScreen(screenSize, box, margin) {
  const slack = margin || 0;
  return (
    box.x + box.w >= -slack &&
    box.x <= screenSize + slack &&
    box.y + box.h >= -slack &&
    box.y <= screenSize + slack
  );
}
