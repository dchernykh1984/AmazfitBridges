...
// The part of a box that is actually on screen, or null when none of it is.
// This is also how the page culls: a bridge that has been dragged off the edge
// clips to nothing, and on a board twice the width of the watch that is more
// than half the widgets skipped on every frame.
//
// A dragged board runs off every edge, which would otherwise mean handing Zepp
// OS widgets at negative coordinates. What the firmware does with those is not
// documented, and the two plausible answers differ badly: clipping draws the
// right picture, while clamping would shove a half-off-screen bridge sideways
// until it no longer met the island it belongs to. Trimming the box here means
// it never has to be found out - the visible result is identical either way.
export function clipToScreen(screenSize, box) {
  const left = Math.max(0, box.x);
  const top = Math.max(0, box.y);
  const right = Math.min(screenSize, box.x + box.w);
  const bottom = Math.min(screenSize, box.y + box.h);
