...
// island whose remaining bridges can only be arranged one way - with no point
...
export function isForcedSolvable(puzzle) {
  ...
  for (;;) {
    ...
    for (let id = 0; id < board.islandCount && forced === null; id++) {
      ...
      const combinations = board.combinationsFor(id);
    ...
    if (board.isSealedOffGroup(forcedIsland)) {
      return false;
    }
  }
}
