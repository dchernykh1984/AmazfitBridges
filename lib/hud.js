...
export function stackLayout(screenSize, items, metrics) {
  ...
  const backdrop = centeredBox(
    screenSize,
    Math.max(0, top - metrics.gap),
    Math.min(screenSize, total + 2 * metrics.gap),
    screenSize,
    0
  );
...
// The rows of the difficulty screen. `hints` is dropped for a board that fits
// the screen whole, because there is nothing to drag.
export function startRows(metrics, showDragHint) {
  const rows = [
    { kind: "text", role: "title", height: metrics.big },
    { kind: "gap", height: metrics.gap },
    { kind: "text", role: "record", height: metrics.small },
    { kind: "gap", height: metrics.gap },
...
// Whether a stacked menu fits on the screen at all. A translation long enough to
// push a menu past the bezel is a bug worth failing a test over, not something
// to discover on a wrist.
export function stackFits(screenSize, items) {
  let total = 0;
  for (let i = 0; i < items.length; i++) {
    total += items[i].height;
  }
  return total <= screenSize;
}
