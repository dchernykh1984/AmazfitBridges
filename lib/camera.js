...
import { playableCells } from "./playfield.js";
...
// A whole world-space box moved into screen space, which is what the page
// actually has to do for every island and every bridge on every drag frame.
export function boxToScreen(camera, box) {
  const origin = toScreen(camera, box.x, box.y);
  return { x: origin.x, y: origin.y, w: box.w, h: box.h };
}

// Whether a disc of the given radius, centred at this screen point, is entirely
// within the round glass.
export function discIsOnScreen(viewSize, x, y, radius) {
...
// player will therefore have to drag to see all of it.
//
// Only the cells a puzzle may actually use are asked about. The corners of the
// grid are the first thing a round screen loses, but no island is ever put
// there - lib/playfield.js keeps them out - so asking about them would answer
// for a part of the board that is always empty, and the smallest size would be
// told to drag when it already fits on the glass whole.
export function needsPanning(layout, viewSize) {
  const camera = centerCamera(layout, viewSize);
  const cells = playableCells(layout.cols, layout.rows);
  for (let i = 0; i < cells.length; i++) {
    const x = (cells[i].col + 0.5) * layout.cell - camera.x;
    const y = (cells[i].row + 0.5) * layout.cell - camera.y;
